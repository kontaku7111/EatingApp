...
public class MFCC {
    ...
    private final static int       n_mfcc       			= 12;
    ...
    public float[] process(double[] doubleInputBuffer) {
        ...
        float [] mfccResult_1D = new float [mfccResult.length];
        for (int m_i = 0; m_i < mfccResult.length; m_i ++){
            mfccResult_1D[m_i]=0;
            for (int m_j = 0; m_j < mfccResult[0].length; m_j ++){
                mfccResult_1D[m_i] += mfccResult[m_i][m_j];
            }
            mfccResult_1D[m_i] = mfccResult_1D[m_i]/mfccResult[m_i].length;
        }
        return mfccResult_1D;
    }
